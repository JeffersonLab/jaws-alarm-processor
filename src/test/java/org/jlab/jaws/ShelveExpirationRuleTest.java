...
public class ShelveExpirationRuleTest {
    ...
    public void tombstoneMsg() throws InterruptedException {
        ...
        keyValues.add(KeyValue.pair(new AlarmOverrideKey("alarm1", OverriddenAlarmType.Shelved), new AlarmOverrideUnion(override1)));
        keyValues.add(KeyValue.pair(new AlarmOverrideKey("alarm1", OverriddenAlarmType.Shelved), new AlarmOverrideUnion(override2)));
        ...
        KeyValue<AlarmOverrideKey, AlarmOverrideUnion> result = outputTopic.readKeyValuesToList().get(0);
        Assert.assertNull(result.value);
    }

    @Test
    public void notYetExpired() {
        inputTopic.pipeInput(new AlarmOverrideKey("alarm1", OverriddenAlarmType.Shelved), new AlarmOverrideUnion(override1));
        testDriver.advanceWallClockTime(Duration.ofSeconds(10));
        inputTopic.pipeInput(new AlarmOverrideKey("alarm2", OverriddenAlarmType.Shelved), new AlarmOverrideUnion(override2));
        KeyValue<AlarmOverrideKey, AlarmOverrideUnion> result = outputTopic.readKeyValuesToList().get(0);
